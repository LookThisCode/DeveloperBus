...
<div id="wrapper" style="width:500px;">
	...
	<?php 
		...
		$rutaGrande = "../db/novedades/";
		
		$query = "SELECT * FROM novedades WHERE borrado=0 AND id=". $id;
		...
		if( isset($_POST['nombre']) && isset($_POST['descripcion'])) {
			...
			if ($nombre != "" &&  strlen($nombre)<=40 && strlen($descripcion)>0 ) {
				...
				if ( !$hayImagenGrande || !$problemaImagenGrande ){
					...
					$query = "UPDATE novedades SET nombre='$nombre', info='$descripcion', imagen='$imagen', visible=$visible WHERE id=$id";
					...
					if (mysql_error() == ''){
						...
						guardarActividad($_SESSION['userActual'], "El usuario ha modificado la novedad: \"$nombre\" ID: $id.");
						redireccionar($to);
					}
					else {
