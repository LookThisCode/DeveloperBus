...
$conn->render_sql("	SELECT productos.*, GROUP_CONCAT( colores.color ORDER BY colores.nombre ASC SEPARATOR  ',' ) AS colores
					FROM productos
					LEFT JOIN coloresproductos ON coloresproductos.idProducto=productos.id
					LEFT JOIN colores ON coloresproductos.idColor=colores.id
					WHERE productos.borrado=0 AND productos.coleccion='Hombre'
