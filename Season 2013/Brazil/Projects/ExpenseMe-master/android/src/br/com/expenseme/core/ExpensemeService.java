...
import static br.com.expenseme.core.Constants.Http.URL_USERS;
...
public class ExpensemeService {
     ...
     * @param password
     */
    public ExpensemeService(final String username, final String password) {
     ...
     * @param apiKey
     */
    public ExpensemeService(final String apiKey, final UserAgentProvider userAgentProvider) {
    ...
    private HttpRequest configure(final HttpRequest request) {
        ...
        if (isPostOrPut(request))
            request.contentType(Constants.Http.CONTENT_TYPE_JSON); // All PUT & POST requests to Parse.com api must be in JSON -
    ...
    private HttpRequest addCredentialsTo(HttpRequest request) {
        ...
        /**
         * Add the user name andh password to the request here if your service needs username or password for each
